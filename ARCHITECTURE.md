...
`listen()` optionally wraps the TCP listener with `netutil.LimitListener` (`--max-conns`). `newServer()` applies conservative timeouts — a short `ReadHeaderTimeout` plus configurable read/write/idle timeouts — to limit resource exhaustion from slow or idle clients. These are defense-in-depth against trivial DoS, independent of the per-IP rate limiter.
